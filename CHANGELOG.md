...
- `claude/CLAUDE.md`: `## Code Graph (codebase-memory-mcp)` section — guidance for querying the pre-indexed code graph via CLI: when to reach for it over grep (transitive consumers via `trace_path`, impact analysis via `detect_changes`, orientation via `get_architecture`) and when not to (point lookups); trust rules treating empty graph results as "unknown" rather than "none" (dynamic imports are invisible, Python edges unreliable, negatives require ripgrep verification); token hygiene (compact tools over `search_graph`, re-index after large refactors)
- `claude/hooks/cbm-code-discovery-gate`, `cbm-session-reminder`, `cbm-subagent-reminder`: hooks installed by codebase-memory-mcp — a PreToolUse augmenter that adds graph context to `Grep`/`Glob` calls (never blocks; fails silent), a SessionStart reminder injecting the code-discovery protocol on startup/resume/clear/compact, and a SubagentStart reminder pointing spawned subagents at the graph tools; the discovery-gate binary path uses `$HOME` instead of the hardcoded home directory the installer wrote
- `claude/skills/codebase-memory/SKILL.md`: skill documenting the knowledge-graph tools — decision matrix mapping questions to tool calls, exploration/tracing workflows, and quality-analysis recipes (dead code, fan-in/fan-out)
- `claude/settings.json`: wire the three cbm hooks into `PreToolUse` (`Grep|Glob`), `SessionStart` (startup/resume/clear/compact), and `SubagentStart`
